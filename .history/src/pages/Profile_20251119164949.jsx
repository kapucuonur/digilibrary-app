...
const Profile = () => {
  ...
  useEffect(() => {
    const loadUserLoans = async () => {
      try {
        ...
        console.log('📚 User loans loaded:', response.data);
  ...
  // Mock user data
  const userData = {
    firstName: user?.firstName || formData.firstName || 'Test',
    lastName: user?.lastName || formData.lastName || 'Kullanıcı',
    email: user?.email || 'test@example.com',
    phone: user?.phone || formData.phone || '+90 555 123 4567',
    joinDate: '2024-01-01',
    totalBooks: 15,
    currentlyReading: userLoans.length // Ödünç alınan kitap sayısı
  };
